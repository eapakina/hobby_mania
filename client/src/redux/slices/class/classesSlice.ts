...
export const classesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(
      getClassesThunk.fulfilled,
      (state, action) => action.payload
    );

    builder.addCase(addClassThunk.fulfilled, (state, action) => [
      ...state,
      action.payload,
    ]);
    ...
    builder.addCase(
      getRandomClassesThunk.fulfilled,
      (state, action) => action.payload
    );
    builder.addCase(
      getFavoriteClassThunk.fulfilled,
      (state, action) => action.payload
    );
  },
});
