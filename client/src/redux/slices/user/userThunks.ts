...
export const removeToken = createAsyncThunk<void>("user/removeToken", () => {
  localStorage.removeItem("token");
});

export const checkUserThunk = createAsyncThunk<UserModelType>(
  "user/checkUser",
  async () => {
    ...
    const { data } = await axios<UserModelType>("/user/check", axiosConfig);
    return data;
  }
);
